##### Functions - Funkcje #####

# countries_information = {}
# countries_information["Polska"] = ("Warszawa", 37.97)
# countries_information["Niemcy"] = ("Berlin", 83.02)
# countries_information["Rosja"] = ("Moskwa", 146.23)
# countries_information["Czechy"] = ("Praga", "brak_danych")
# countries_information["Slowacja"] = ("Bratyslawa", 5.45)

# def show_country_info(country):
#     country_information = countries_information.get(country)

#     print()
#     print(country)
#     print("__________")
#     print("Stolica: " + country_information[0])
#     print("Liczba_mieszkancow (mln): " + str(country_information[1]))


# for country in countries_information.keys():
#     print(country)

# country = input("Info o jakim kraju chcesz wyswietlic? ")
# show_country_info(country)



def display_sum(a, b):
    print(a+b)

def calculate_sum(a,b):
    return a+b  

def print_message():
    print("Super Wiadomosc")

def print_message():
    print("Taka sobie informacja")

sum = calculate_sum(2,3)
print(sum) 
print_message() 

